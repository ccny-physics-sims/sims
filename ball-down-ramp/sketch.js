...
canvas.parent('sketch-holder');
  ...
  posRoller = createVector(.85*width,.8*height-.85*width*tan(rampTheta))
...
function draw(){
  ...
  translate(0,20)
  rotate(-rampTheta);
  line(0,posRoller.y+distancetozero*tan(rampTheta)-12, width, posRoller.y+distancetozero*tan(rampTheta)-12)
  //line(posRoller.x,posRoller.y,width,posRoller.y-(width*tan(rampTheta)));
  pop();
  if (ballRoller.position.x < .1*width) {
    //resetTheBalls();
