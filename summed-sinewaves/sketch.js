...
function drawSine(counter_,strokeColor_) {
    x = 0;
    frequencyScaler = map(counter_,1,100,1,2)
  for (var i = 0; i < yvalues.length; i++) {
    //yvalues[i] =(4 / Math.PI) * (1 / ((counter_*2)-1)) * Math.sin(((counter_*2)-1) * Math.PI * x / wavelength);
    yvalues[i] = 020*Math.sin(frequencyScaler * x * k -frequencyScaler*60) ;/// (((2*counter_)-1));
    //yvalues[i] = 100*Math.sin( x * k ) ;
  ...
  for (var x = 0; x < yvalues.length; x++) {
    curveVertex(x*xspacing, height/2+yvalues[x]-200);
  }
...
function drawSum(howMany) {
  ...
  beginShape();
    for (var x = 0; x < sumedFunction.length; x++) {
      scaledValue = map(sumedFunction[x],min(sumedFunction),max(sumedFunction),-100,100)
      curveVertex(x*xspacing, height/2+(scaledValue));
    }
  endShape();
  pop();
//   push();
